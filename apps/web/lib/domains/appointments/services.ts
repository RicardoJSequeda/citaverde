...
import { cacheKeys, getCached, setCached, deleteCached, invalidateCachePattern } from "@/lib/cache/redis"
...
export async function getAvailableSlots(
  ...
  try {
    // Check cache first
    const cacheKey = cacheKeys.availableSlots(professionalId, date)
    const cachedSlots = await getCached<string[]>(cacheKey)
    if (cachedSlots) {
    ...
    if (!schedule) {
      await setCached(cacheKey, [], 1800) // Cache empty slots for 30 min
      return { slots: [] }
    }
    ...
    await setCached(cacheKey, slots, 1800)
    return { slots }
  } catch (error) {
...
export async function createAppointmentService(
  ...
  try {
    ...
    // Invalidate cache
    await invalidateCachePattern(`slots:${formData.professionalId}:*`)

    // Queue notification (will be processed async)
...
export async function cancelAppointmentService(appointmentId: string, userId: string, reason: string) {
  try {
    const supabase = await createServerClient()
    ...
    // Invalidate cache
    await invalidateCachePattern(`slots:${appointment.professional_id}:*`)
...
export async function checkInAppointmentService(qrCode: string, userId: string) {
  try {
    const supabase = await createServerClient()
    ...
    return { error: "Failed to check in appointment" }
  }
}
...
export async function reissueQRCodeService(appointmentId: string, userId: string) {
  try {
    const supabase = await createServerClient()
    ...
    // Queue notification with new QR
    await supabase.from("notifications").insert({
      user_id: userId,
      type: "qr_reissued",
      channel: "email",
...
export async function rateAppointmentService(
  appointmentId: string,
  userId: string,
  rating: number,
  feedback?: string,
