...
export function RealtimeReceptionistQueue({ initialWaitingQueue, initialCalledQueue }: RealtimeReceptionistQueueProps) {
  const allTickets = useRealtimeSubscription('queue_tickets', [...initialWaitingQueue, ...initialCalledQueue]);
  ...
  const calledQueue = useMemo(
    () =>
      allTickets
        ...
        .sort((a: any, b: any) => new Date(b.called_at || 0).getTime() - new Date(a.called_at || 0).getTime()),
    [allTickets],
  )
